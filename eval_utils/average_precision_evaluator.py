...
class Evaluator:
    ...
    def write_predictions_to_txt(self,
        ...
        for class_id in range(1, self.n_classes + 1):
            ...
            if classes is None:
                ...
                class_suffix = classes[int(class_id)]

            results_file = open('{}{}.txt'.format(out_file_prefix, class_suffix), 'w')

            for prediction in self.prediction_results[int(class_id)]:
    ...
    def match_predictions(self,
        ...
        for class_id in range(1, self.n_classes + 1):
            ...
            predictions = self.prediction_results[int(class_id)]
    ...
    def compute_precision_recall(self, verbose=True, ret=False):
        ...
        for class_id in range(1, self.n_classes + 1):
            ...
            tp = self.cumulative_true_positives[int(class_id)]
            fp = self.cumulative_false_positives[int(class_id)]


            cumulative_precision = np.where(tp + fp > 0, tp / (tp + fp), 0) # 1D array with shape `(num_predictions,)`
            cumulative_recall = tp / self.num_gt_per_class[int(class_id)] # 1D array with shape `(num_predictions,)`
    ...
    def compute_average_precisions(self, mode='sample', num_recall_points=11, verbose=True, ret=False):
        ...
        for class_id in range(1, self.n_classes + 1):
            ...
            cumulative_precision = self.cumulative_precisions[int(class_id)]
            cumulative_recall = self.cumulative_recalls[int(class_id)]
